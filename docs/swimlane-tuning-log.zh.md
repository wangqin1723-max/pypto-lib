...
> 起因：泳道图中段全是「细碎的核」。**第 1 节的逐 batch coarsening 改不了**——[pypto-lib#405](https://github.com/hw-native-sys/pypto-lib/pull/405) 之后 state/cache 走分页 block-table，16 个 batch 落在 16 个不相邻物理块，拼不成连续 tile（即"上面的 api 不能动"）。
...
>
> 验证备注：上表为跨 session 各一跑，Total 含 ~7% 抖动，跨 session 对比仅作辅助参考；qr_rope 的 256→64 + 聚合 Exec 砍半是确定的结构性归因。CSA 整图两次都 507018（已知间歇性 orchestrator 死锁，**非本改动**——standalone 稳定 PASS 即证）；要 CSA 新泳道需重试 `decode_attention_csa.py` 直到一次不 507018。
...
> 依赖两个 codegen 修复才能把 cube→vec 融进一个 task：原修复编号 **`#1761`**（SplitVectorKernel 把 `split=0` 的 cube↔vec pipe 复制进两个 AIV subblock → 单生产双消费死锁 507018）、**`#1768`**（orchestration 把按动态 `t_dim` 定大小的注入式 GM pipe buffer 排在 `t_dim` 声明之前 → use-before-decl 编不过）。
...
下游(内联 hc_pre)在 main 版上复验无回归:`decode_attention_csa.py`(kv_cache+x_out PASS)、`moe_ep.py`(x_next PASS, 2-rank)。PR: [pypto-lib#533](https://github.com/hw-native-sys/pypto-lib/pull/533)。
...
机理参考（D_TILE=512、AIV 96us 那次）：`build_output/_jit_hc_pre_test_20260616_122317/dfx_outputs/merged_swimlane_20260616_122323.json`。PR: [pypto-lib#545](https://github.com/hw-native-sys/pypto-lib/pull/545)。
...
> 验证 build：baseline `build_output/_jit_compressor_test_20260626_173729`，widen `..._172915`（dfx_outputs/merged_swimlane_*.json）。解析法：读 traceEvents 取 `ph=="X"` 且 name 以 `softmax_pool(` 开头的事件，`busy=Σdur`、`span=max(ts+dur)−min(ts)`。已提 PR [pypto-lib#624](https://github.com/hw-native-sys/pypto-lib/pull/624)。
...
**standalone `decode_compressor_ratio4`**（2 跑干净，真机 a2a3，rebase 到含 [pypto-lib#624](https://github.com/hw-native-sys/pypto-lib/pull/624) widen 的 main 上）：
...
>
> **排查弯路（记一笔，省下次的命）**：传播后 CI 上板报 `Subscript-write shape mismatch: window expects 0, source has 128`，定位发现是 **`decode_attention_swa.py:180`（跟 b_trans 无关！）**——我的分支基于**旧 base**带旧 SWA，在**新 pypto 的严检查**下越界；新 main（[pypto-lib#629](https://github.com/hw-native-sys/pypto-lib/pull/629) SWA 重写）不会。decode_fwd/decode_layer 走 SWA 才中招，compressor/CSA 不碰 SWA 所以一直绿。**修复 = rebase 到新 main**，不是改 b_trans。教训：① 报错文件名要看准（别假设是自己改的文件）；② 本地复现不出 CI 报错先查 **pypto 版本偏差**（CI 用 `ci.yml` pin 的 origin HEAD，本地可能落后；`python -c "import pypto;print(pypto.__file__)"` + `git -C <pypto> rev-list --count HEAD..origin/main`）；③ 本地编译干净 ≠ CI 干净（CI `rm -rf build_output` 全新 trace，本地有缓存）。
>
> 验证 build：standalone ND `_jit_compressor_test_20260627_115352`、b_trans `..._115315`；CSA 3 跑见 `csa_{btrans,ndbase}_x3_626.log`。PR [pypto-lib#628](https://github.com/hw-native-sys/pypto-lib/pull/628)（`perf/dsv4-compressor-btrans-load`，a2a3 上板 CI 绿；sim 不看护、忽略）。probe：`models/deepseek/v4/_tmp_nz_weight_probe.py`（`PROBE_NZ`/`PROBE_TRANS`）。

![alt text](img_v3_02132_71914fdb-c67c-427d-8d12-7860b9cd279g-public.png)![alt text](image-15-public.png)
...
> 验证 build：原始 `_jit_attention_hca_test_20260629_150217`、R1 `..._161532`、R2 `..._165013`（`dfx_outputs/l2_swimlane_records.json`）；incore `build_output/incore_softmax_pool_ratio128_blockload_20260629`（R1）、`..._colreduce_ht128_20260629`（R2，含 `summary.txt` 记录复现条件）。PR [pypto-lib#641](https://github.com/hw-native-sys/pypto-lib/pull/641)（`perf/dsv4-compressor-softmax-pool-blockload`，基于 main，不依赖在途的 [pypto-lib#628](https://github.com/hw-native-sys/pypto-lib/pull/628) b_trans 分支——改的是不同行）。


![alt text](image-20-public.png)
![alt text](image-21-public.png)
![alt text](image-22-public.png)
三次的泳道图对比：
