...
# -----------------------------------------------------------------------------------------------------------
# ci: devices=8  # CI: fixed EP8 benchmark
# ci: no-sim    # CI marker: full multi-layer / multi-card forward — device-only, skip on *sim
"""DeepSeek-V4 Flash host-prepared EPLB decode through FP32 logits."""
...
# HCA and CSA keep separate compressed KV pools; their block counts may match
# while their storage block sizes differ by compression ratio.

# Per-layer packed cache pools with runtime block extents.
...
CSA_NUM_LAYERS = 21
HCA_NUM_LAYERS = 20
# Final index in the forward-layer weight stack.
FWD_LAST_LAYER = FWD_NUM_LAYERS - 1
# LM-head completion epoch.
...
# Per-layer paged KV and compressor-state pools initialized from one tiled layer.
...
# Rank-stacked resident weights, including rank-local LM-head vocabulary shards.
...
# Rank-stacked resident KV and compressor-state pools updated in place.
RESIDENT_CACHE_OUTPUT_NAMES = CACHE_POOL_NAMES


@pl.jit.inline(auto_scope=False)
def eplb_decode_logits_inline(
    ...
    hca_cmp_kv: pl.InOut[pl.Tensor[[FWD_CMP_BLOCK_NUM_DYN, HCA_CMP_STORAGE_BLOCK_SIZE, 1, HEAD_DIM], pl.BF16]],
    csa_cmp_kv: pl.InOut[pl.Tensor[[FWD_CMP_BLOCK_NUM_DYN, CSA_CMP_STORAGE_BLOCK_SIZE, 1, HEAD_DIM], pl.BF16]],
    ...
    cmp_block_num = pl.tensor.dim(csa_cmp_kv, 0) // FWD_NUM_LAYERS
    ...
    with pl.scope():
        moe(
            ...
            recv_meta, recv_x, recv_aux, recv_route, arrived, data_arrived,
            routed_y_buf, combine_arrived,
            pl.cast(0, pl.INT32), nt, my_rank, pl.cast(2, pl.INT32),
        )
    for loop_i in pl.range(HCA_NUM_LAYERS):
        ...
        cmp_kv_csa = pl.slice(csa_cmp_kv, [cmp_block_num, CSA_CMP_STORAGE_BLOCK_SIZE, 1, HEAD_DIM], [csa_layer * cmp_block_num, 0, 0, 0])
        ...
        cmp_kv_hca = pl.slice(hca_cmp_kv, [cmp_block_num, HCA_CMP_STORAGE_BLOCK_SIZE, 1, HEAD_DIM], [hca_layer * cmp_block_num, 0, 0, 0])
    ...
    # Final forward-stack and CSA-stack indices.
    ...
    cmp_kv_last = pl.slice(csa_cmp_kv, [cmp_block_num, CSA_CMP_STORAGE_BLOCK_SIZE, 1, HEAD_DIM], [csa_layer_last * cmp_block_num, 0, 0, 0])
...
def eplb_decode_logits(
    ...
    return eplb_decode_logits_inline(
        x_hc,
        hc_attn_fn,
        hc_attn_scale,
        hc_attn_base,
...
def l3_eplb_decode_logits(
    ...
    hca_cmp_kv: pl.InOut[pl.Tensor[[N_RANKS, FWD_CMP_BLOCK_NUM_DYN, HCA_CMP_STORAGE_BLOCK_SIZE, 1, HEAD_DIM], pl.BF16]],
    csa_cmp_kv: pl.InOut[pl.Tensor[[N_RANKS, FWD_CMP_BLOCK_NUM_DYN, CSA_CMP_STORAGE_BLOCK_SIZE, 1, HEAD_DIM], pl.BF16]],
    ...
    # Shared MoE/LM-head data windows and separate completion counters.
    ...
    # LM-head hidden/logit windows and completion counters.
    ...
    for r in pl.range(pld.world_size()):
        ...
        eplb_decode_logits(
            ...
            csa_inner_compress_state[r], hca_cmp_kv[r], csa_cmp_kv[r], idx_kv_cache[r], idx_kv_scale[r],
            ...
            csa_idx_slot_mapping[r], csa_state_slot_mapping[r], csa_inner_state_slot_mapping[r],
            position_ids[r],
...
# Kernel-only stacked fixture specs.
def _make_layer_stacked_spec(name, base_specs, layer_count=FWD_NUM_LAYERS):
    ...
    if name == "tid2eid":
        return make_eplb_tid2eid_spec(spec, layer_count=layer_count)
    packed_shape = [spec.shape[0], layer_count * spec.shape[1], *spec.shape[2:]]

    def init_value():
        base_init = spec.init_value
        if name in CACHE_POOL_NAMES:
            # Tile one initialized cache layer across the packed layer axis.
            one_layer = base_init()
    ...
    return TensorSpec(
        ...
        # Mark kernel-written resident caches as outputs.
...
def build_single_layer_tensor_specs(
    ...
    freqs_cos_spec = TensorSpec(
        "freqs_cos", [2, *swa_specs["freqs_cos"].shape], swa_specs["freqs_cos"].dtype,
        init_value=init_freqs_cos,
    )
    freqs_sin_spec = TensorSpec(
...
def build_tensor_specs(
    ...
    ordered_names = [
        ...
        "wq_a", "wq_b", "wq_b_scale", "wkv", "gamma_cq", "gamma_ckv", "kv_cache",
        "attn_sink", "wo_a", "wo_b", "wo_b_scale",
        "hca_cmp_wkv", "hca_cmp_wgate", "hca_cmp_ape", "hca_cmp_norm_w", "hca_compress_state",
        "csa_cmp_wkv", "csa_cmp_wgate", "csa_cmp_ape", "csa_cmp_norm_w", "csa_compress_state",
        "csa_idx_wq_b", "csa_idx_wq_b_scale", "csa_weights_proj", "csa_hadamard_idx",
        "csa_inner_wkv", "csa_inner_wgate", "csa_inner_ape", "csa_inner_norm_w",
        "csa_inner_compress_state", "hca_cmp_kv", "csa_cmp_kv", "idx_kv_cache", "idx_kv_scale",
        ...
        "csa_inner_state_slot_mapping", "position_ids", "kv_seq_lens",
        "hca_compress_state_block_table", "csa_compress_state_block_table",
        "csa_inner_compress_state_block_table", "cmp_block_table", "idx_block_table", "input_ids",
    ...
    for name in ordered_names:
        if name in metadata_specs:
            ...
            specs.append(make_eplb_input_ids_spec(base_specs[name], active_tokens=num_tokens))
            ...
            raise ValueError(f"unclassified eplb_decode_logits spec: {name}")

    # Rank-stack resident weights and in-place cache pools.
    for spec in specs:
...
def main():
    parser = argparse.ArgumentParser(description="DeepSeek-V4 Flash host-prepared EPLB decode-to-logits benchmark.")
    ...
    parser.add_argument("--experts-per-rank", type=int, default=EPLB_EXPERTS_PER_RANK, choices=[EPLB_EXPERTS_PER_RANK])
    ...
    if len(device_ids) != EPLB_EP_SIZE:
        raise ValueError(f"EPLB benchmark requires exactly {EPLB_EP_SIZE} devices, got {device_ids}")
