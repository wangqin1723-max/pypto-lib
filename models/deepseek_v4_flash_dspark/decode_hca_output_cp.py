...
# -----------------------------------------------------------------------------------------------------------
# ci: devices=4
"""DeepSeek-V4 HCA decode DSA-CP attention output half."""
...
if T_PAD != LOCAL_T_PAD:
    raise ValueError(f"HCA token capacity {T_PAD} must equal CP local token capacity {LOCAL_T_PAD}")
if FIXTURE_WINDOW_BLOCKS > ORI_BLOCK_NUM:
    raise ValueError("HCA fixture window exceeds the original KV cache capacity")


@pl.jit
def decode_hca_output_cp(
...
def l3_decode_hca_output_cp(
    ...
    cmp_kv: pl.Tensor[[SP_SIZE, CMP_BLOCK_NUM, BLOCK_SIZE, 1, HEAD_DIM], pl.BF16],
    cmp_block_table: pl.Tensor[[SP_SIZE, B_DYN, CMP_MAX_BLOCKS], pl.INT32],
    cmp_sparse_indices: pl.Tensor[[SP_SIZE, T_DYN, CMP_TOPK], pl.INT32],
    ...
    """Launch the HCA output half on one physical SP group."""
...
def build_tensor_specs(local_t):
    """Build deterministic four-rank HCA output-half inputs."""
    import torch

    from golden import ScalarSpec, TensorSpec

    if local_t < 8 or local_t > LOCAL_T or local_t % 8 != 0 or local_t % S != 0:
        raise ValueError(f"local_t must be a multiple of 8 in [8, {LOCAL_T}], got {local_t}")
    local_batch = local_t // S
    cmp_block_table_shape = [SP_SIZE, local_batch, CMP_MAX_BLOCKS]
    ...
    def init_ori_kv():
        shape = (SP_SIZE, ORI_BLOCK_NUM, BLOCK_SIZE, 1, HEAD_DIM)
    ...
    def init_cmp_kv():
        shape = (SP_SIZE, CMP_BLOCK_NUM, BLOCK_SIZE, 1, HEAD_DIM)
        return torch.full(shape, float("nan"), dtype=torch.bfloat16)

    def init_cmp_block_table():
        return torch.zeros(cmp_block_table_shape, dtype=torch.int32)

    def init_cmp_sparse_indices():
        return torch.full((SP_SIZE, local_t, CMP_TOPK), -1, dtype=torch.int32)
    ...
    return [
        ...
        TensorSpec("cmp_kv", [SP_SIZE, CMP_BLOCK_NUM, BLOCK_SIZE, 1, HEAD_DIM], torch.bfloat16, init_value=init_cmp_kv),
        TensorSpec("cmp_block_table", cmp_block_table_shape, torch.int32, init_value=init_cmp_block_table),
        TensorSpec("cmp_sparse_indices", [SP_SIZE, local_t, CMP_TOPK], torch.int32, init_value=init_cmp_sparse_indices),
...
def golden_decode_hca_output_cp(tensors):
...
if __name__ == "__main__":
    ...
    for case in selected_cases:
        ...
        result = run_jit(
            ...
            compare_fn={
                "o_local": build_o_local_compare(local_t),
            },
        )
