...
# -----------------------------------------------------------------------------------------------------------
"""Qwen3-32B prefill Scope 1+2 — with I/O buffer caching (via io_cache plugin).

Identical kernel program to qwen3_32b_prefill_scope12.py.  All program,
...
Usage:
  python qwen3_32b_prefill_scope12_wIOBuffer.py -p a2a3 -d 0
  python qwen3_32b_prefill_scope12_wIOBuffer.py -p a2a3 -d 0 --clear-cache
"""
from qwen3_32b_prefill_scope12 import (
    build_prefill_scope12_program,
    build_tensor_specs,
    golden_prefill_scope12,
    BATCH, MAX_SEQ, HIDDEN, NUM_HEADS, NUM_KV_HEADS, HEAD_DIM,
)
...
def compile_and_run(
    batch=BATCH, max_seq=MAX_SEQ, hidden_size=HIDDEN,
    num_heads=NUM_HEADS, num_kv_heads=NUM_KV_HEADS, head_dim=HEAD_DIM,
    use_max_seq=False,
    ...
    program = build_prefill_scope12_program(
        batch=batch, max_seq=max_seq, hidden_size=hidden_size,
        num_heads=num_heads, num_kv_heads=num_kv_heads, head_dim=head_dim,
    )
    ...
    golden = wrap_golden(golden_prefill_scope12, "attn_out", cache_dir, tensor_specs)
